...
class ImmomoAdapter(private val service: AccessibilityService) : PlatformAdapter {
    ...
    override fun isInChat(root: AccessibilityNodeInfo): Boolean {
        val nodes = root.findAccessibilityNodeInfosByViewId("/chat_input")
        return nodes.any { it.isVisibleToUser }
    }

    override fun isInMessageList(root: AccessibilityNodeInfo): Boolean {
        val nodes = root.findAccessibilityNodeInfosByViewId("/message_list")
        return nodes.isNotEmpty()
    }

    override fun readMessages(root: AccessibilityNodeInfo): List<ChatMessage> {
        val messages = mutableListOf<ChatMessage>()
        val containers = root.findAccessibilityNodeInfosByViewId("/chat_item")
    ...
    override suspend fun fillAndSend(service: AccessibilityService, root: AccessibilityNodeInfo, text: String): SendResult {
        val inputNodes = root.findAccessibilityNodeInfosByViewId("/chat_input")
        ...
        val sendNodes = root.findAccessibilityNodeInfosByViewId("/send_btn")
