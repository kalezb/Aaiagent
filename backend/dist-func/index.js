...
// ../node_modules/.pnpm/hono@4.13.8/node_modules/hono/dist/utils/buffer.js
...
// ../node_modules/.pnpm/hono@4.13.8/node_modules/hono/dist/utils/url.js
...
// ../node_modules/.pnpm/hono@4.13.8/node_modules/hono/dist/request.js
...
// ../node_modules/.pnpm/hono@4.13.8/node_modules/hono/dist/utils/html.js
...
// ../node_modules/.pnpm/hono@4.13.8/node_modules/hono/dist/context.js
...
// ../node_modules/.pnpm/hono@4.13.8/node_modules/hono/dist/router.js
...
// ../node_modules/.pnpm/hono@4.13.8/node_modules/hono/dist/utils/constants.js
var COMPOSED_HANDLER = "__COMPOSED_HANDLER";

// ../node_modules/.pnpm/hono@4.13.8/node_modules/hono/dist/hono-base.js
...
// ../node_modules/.pnpm/hono@4.13.8/node_modules/hono/dist/router/utils.js
var createNullObject = /* @__PURE__ */ __name(() => /* @__PURE__ */ Object.create(null), "createNullObject");

// ../node_modules/.pnpm/hono@4.13.8/node_modules/hono/dist/router/reg-exp-router/matcher.js
var emptyParam = [];
...
// ../node_modules/.pnpm/hono@4.13.8/node_modules/hono/dist/router/reg-exp-router/node.js
...
// ../node_modules/.pnpm/hono@4.13.8/node_modules/hono/dist/router/reg-exp-router/trie.js
...
// ../node_modules/.pnpm/hono@4.13.8/node_modules/hono/dist/router/reg-exp-router/router.js
...
// ../node_modules/.pnpm/hono@4.13.8/node_modules/hono/dist/router/smart-router/router.js
...
// ../node_modules/.pnpm/hono@4.13.8/node_modules/hono/dist/router/trie-router/node.js
...
// ../node_modules/.pnpm/hono@4.13.8/node_modules/hono/dist/router/trie-router/router.js
...
// ../node_modules/.pnpm/hono@4.13.8/node_modules/hono/dist/hono.js
...
// ../node_modules/.pnpm/hono@4.13.8/node_modules/hono/dist/middleware/cors/index.js
...
async function getRecentHistory(db, token, platform, contactId) {
  ...
  ).bind(token, platform, contactId, MAX_MESSAGES + 10).all();
  if (!results || results.length === 0) {
    return { messages: [], summary, hoursAgo: 0 };
  }
  const allMessages = results.reverse();
  let hoursAgo = 0;
  for (let i = allMessages.length - 1; i >= 0; i--) {
    if (allMessages[i].role === "user") {
      hoursAgo = Math.floor((Date.now() / 1e3 - allMessages[i].created_at) / 3600);
      break;
    }
  }
  let messages;
  if (allMessages.length <= MAX_MESSAGES) {
    messages = allMessages.map((m) => ({ role: m.role, content: m.content, created_at: m.created_at }));
  } else {
...
async function maybeRegenerateSummary(db, token, platform, contactId, deepseekKey) {
  try {
    const summaryRow = await db.prepare(
      "SELECT summary, summarized_up_to_id FROM session_summary WHERE token = ? AND platform = ? AND contact_id = ?"
    ).bind(token, platform, contactId).first();
    ...
    if (maxId - lastSummarizedId < 10)
      return;
    const { results } = await db.prepare(
      `SELECT role, content FROM chat_history
       WHERE token = ? AND platform = ? AND contact_id = ? AND id > ?
       ORDER BY created_at ASC LIMIT 20`
    ).bind(token, platform, contactId, lastSummarizedId).all();
    if (!results || results.length === 0)
      return;
    const conversationText = results.map((m) => (m.role === "user" ? "\u5BF9\u65B9" : "\u6211") + ": " + m.content).join("\n");
    ...
    await db.prepare(
      `INSERT INTO session_summary (token, platform, contact_id, summary, summarized_up_to_id)
       ...
       ON CONFLICT(token, platform, contact_id) DO UPDATE SET
         summary = excluded.summary, summarized_up_to_id = excluded.summarized_up_to_id`
    ).bind(token, platform, contactId, newSummary.substring(0, 200), maxId).run();
  } catch (error) {
    console.error("Summary generation error:", error);
  }
}
__name(maybeRegenerateSummary, "maybeRegenerateSummary");
function buildLLMMessages(systemPrompt, historyMessages, summary, newMessages) {
  const result = [
    { role: "system", content: systemPrompt }
  ];
  if (summary) {
    result.push({ role: "user", content: `\u4E4B\u524D\u7684\u804A\u5929\u5927\u6982\u662F\u8FD9\u6837\uFF1A${summary}` });
  }
  for (const msg of historyMessages) {
    const timeStr = new Date(msg.created_at * 1e3).toLocaleString("zh-CN", {
      hour: "2-digit",
      minute: "2-digit"
    });
    const roleLabel = msg.role === "user" ? "\u5BF9\u65B9\u8BF4" : "\u4F60\u8BF4";
    result.push({ role: "user", content: `[${timeStr}] ${roleLabel}\uFF1A${msg.content}` });
  }
...
\u4ECA\u5929${weather.condition}\uFF0C\u6C14\u6E29${weather.temp}\xB0C\u3002
...
2. \u53EA\u56DE\u590D\u5BF9\u65B9\u6700\u65B0\u7684\u8FD9\u6761\u6D88\u606F\uFF0C\u57FA\u4E8E\u4E0A\u4E0B\u6587\u81EA\u7136\u63A5\u8BDD\u3002`;
    ...
    const batch = messages.map(
      (msg) => stmt.bind(tokenRow.token, platform, contact_id, contact_name, msg.role, msg.content, msg.created_at || nowSec)
    );
    ...
    ).bind(nowSec, tokenRow.token).run();
...
messagesRouter.post("/sync", async (c) => {
  try {
    ...
    const batch = messages.map(
      (msg) => stmt.bind(token, platform, contact_id, contact_name, msg.role, msg.content, msg.created_at || nowSec)
    );
    await c.env.DB.batch(batch);
    await c.env.DB.prepare(
...
historyRouter.delete("/", async (c) => {
  try {
    ...
    ).bind(token, platform, contact_id).run();
...
personaRouter.put("/", async (c) => {
  try {
    ...
    ).bind(id, token || "", name, system_prompt, isActive, now).run();
    return c.json({ success: true }, 200);
  } catch (error) {
...
configRouter.put("/", async (c) => {
  try {
    const body = await c.req.json();
    if (body.model_name)
      await c.env.KV.put("llm:model", body.model_name);
    if (body.api_base)
      await c.env.KV.put("llm:base_url", body.api_base);
    if (body.temperature !== void 0)
      await c.env.KV.put("llm:temperature", String(body.temperature));
    if (body.max_tokens !== void 0)
      await c.env.KV.put("llm:max_tokens", String(body.max_tokens));
    return c.json({ success: true }, 200);
  } catch (error) {
  ...
  try {
    ...
    await c.env.DB.prepare(
      `INSERT INTO tokens (token, name, monthly_limit, spent, is_active, created_at, last_used_at)
       VALUES (?, ?, ?, 0, 1, ?, ?)`
    ).bind(token, name, monthlyLimit, now, now).run();
...
// ../.wrangler/tmp/pages-vqptUu/functionsRoutes-0.295862011444058.mjs
var routes = [
  {
    routePath: "/api/:route*",
    mountPath: "/api",
...
// ../node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260702.1/node_modules/wrangler/templates/pages-template-worker.ts
var escapeRegex = /[.+?^${}()|[\]\\]/g;
