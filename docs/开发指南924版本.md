...
> 所有权限在前端自由开启，所有配置前端可改，后端必须按前端配置执行。

#### 卡片1：设备权限

| 按钮 | 作用 | 状态显示 |
|------|------|---------|
| 无障碍读取与操作 | 读取消息 + 自动操作 | 绿色=已授权 / 灰色=未授权 |
| 通知读取 | 监听新消息通知 | 绿色=已授权 / 灰色=未授权 |
| 电池优化白名单 | 防止后台被杀 | "去设置"按钮，点了跳系统设置 |
| 悬浮窗权限 | 显示悬浮球 | "去设置"按钮，点了跳系统设置 |

#### 卡片2：平台与人设

| 按钮 | 作用 |
|------|------|
| 平台选择（Soul/QQ/连信/陌陌） | 选哪个平台就盯哪个，单选 |
| 客服人设（下拉选择） | 选好人设，后端大模型必须按这个人设说话 |
| 城市（输入框） | 你填什么城市，大模型就说自己在哪个城市 |
| 区域/地标（输入框） | 你填什么区域，大模型就说自己在哪个区域 |
| 保存位置按钮 | 点了保存，后端必须按你填的位置说 |

**底部显示当前状态：** 当前：Soul | 星暮·女·29岁 | 重庆两江新区

#### 卡片3：AI托管控制

| 按钮 | 作用 |
|------|------|
| AI托管开关 | 开了就自动回消息，关了就停 |
| 监控模式开关 | 开了就实时同步聊天记录到后端，不管AI托管开没开 |
| 发送方式：自动发送 | AI想完直接发，不用你确认 |
| 发送方式：手动确认 | AI想完先给你看，你点确认才发 |
| 联系人白名单管理 | 只有白名单里的人，AI才回 |
| 联系人黑名单管理 | 黑名单里的人，AI绝对不回 |

#### 卡片4：位置设置（家庭+工作双地址）

| 按钮 | 作用 |
|------|------|
| 家庭地址（城市+区域） | 你填什么，大模型就说自己住在哪 |
| 工作地址（城市+区域） | 你填什么，大模型就说自己在哪上班 |
| 保存按钮 | 点了保存，两个地址同时生效 |

**大模型自己判断：** 对方问"你家在哪？"就说家庭地址，对方问"你在哪上班？"就说工作地址。

#### 卡片5：设置

| 按钮 | 作用 |
|------|------|
| 设备钥匙 | 显示你的设备钥匙（mykey_xxx） |
| 天气开关 | 开了大模型知道今天天气 |
| 时间感知开关 | 开了大模型知道现在几点，不会早上回"晚上好" |
...
| 消息列表 RecyclerView | cn.soulapp.android:id/conversation_list |
| 会话项 | cn.soulapp.android:id/item_content_root |
| 对方昵称 | cn.soulapp.android:id/name |
| 最后一条消息 | cn.soulapp.android:id/message |
| 时间 | cn.soulapp.android:id/time |
| 未读数字 | cn.soulapp.android:id/unread_msg_number |
| 头像 | cn.soulapp.android:id/avatar |

### 14.2 聊天页里要找的东西

| 元素 | View ID | 说明 |
|------|---------|------|
| 输入框 | cn.soulapp.android:id/et_sendmessage | 输入文字的地方 |
| 消息项 | cn.soulapp.android:id/item_root | 每条消息的根节点 |
| 文字容器 | cn.soulapp.android:id/content_text | 文字消息的文本 |
| 时间戳 | cn.soulapp.android:id/timestamp | 时间分隔 |
| 对方头像 | cn.soulapp.android:id/otherAvatar | 左边的头像 |
| 我的头像 | cn.soulapp.android:id/meAvatar | 右边的头像 |
| 顶部标题 | cn.soulapp.android:id/tv_title | 对方昵称 |
| 语音气泡 | cn.soulapp.android:id/voice_bubble | 语音消息 |
| 语音时长 | cn.soulapp.android:id/tv_length | 显示 "2''"、"3''" |
| 图片 | cn.soulapp.android:id/image | 图片消息 |
| 亲密表情（骰子） | cn.soulapp.android:id/gif_intimacy | 骰子/亲密表情 |
| 隐私保护标签 | cn.soulapp.android:id/tv_privacy_protect_tag | 隐私保护图 |
| 交换图片消息项 | cn.soulapp.android:id/item_roote_snap_exchange_photo | 闪图/交换图片 |
| AI 建议卡片 | cn.soulapp.android:id/aigcRootView | **过滤掉！** |

### 14.3 消息类型识别（Soul 版）

| 类型 | 怎么判断 | 怎么处理 |
|------|---------|---------|
| **文字消息** | 有 content_text | 直接读文字，调 LLM |
| **语音消息** | 有 oice_bubble，content-desc 是 "语音消息" | 长按 → 点"转文字" → 读结果 |
| **图片消息** | 有 image | 截图 → 视觉模型 |
| **表情包/骰子** | 有 gif_intimacy | 截图 → 视觉模型 |
| **隐私保护图** | 有 	v_privacy_protect_tag | 跳过，不回复 |
| **交换图片/闪图** | 有 item_roote_snap_exchange_photo | 跳过，不回复 |
| **时间分隔** | 有 	imestamp | 跳过，不回复 |
| **AI 建议卡片** | 有 igcRootView | **过滤掉！** |

### 14.4 注意事项

1. **Soul 默认打开是广场页**，不是消息列表。要导航到消息列表页。
2. **AI 建议卡片要过滤掉**，不要当成对方发的消息。
3. **隐私保护图和交换图片要跳过**，不要回复。
4. **语音转文字用长按的方式**，不要用语音旁边的"转文字"按钮。
...
**菜单是自绘浮层**，uiautomator抓不到，所以靠UI树找"转文字"按钮是行不通的。只能靠截图+视觉模型识别"转文字"三个字的位置。
...
class SoulVoiceTranscriber(
    ...
    private fun findVoiceMessages(root: AccessibilityNodeInfo): List<AccessibilityNodeInfo> {
        ...
        for (item in messageItems) {
            val voiceBubble = findNodeById(item, "cn.soulapp.android:id/voice_bubble")
            if (voiceBubble != null && voiceBubble.contentDescription == "语音消息") {
    ...
    private fun getVoiceDuration(node: AccessibilityNodeInfo): String? {
        val lengthNode = findNodeById(node, "cn.soulapp.android:id/tv_length")
        return lengthNode?.text?.toString()
    }
    
    private fun longPressVoiceMessage(voiceNode: AccessibilityNodeInfo) {
        val bounds = Rect()
        voiceNode.getBoundsInScreen(bounds)
        performLongClick(bounds.centerX(), bounds.centerY(), 2000)
    }
    
    private suspend fun findTranscribeButtonByVision(): Point? {
    ...
    private suspend fun getTranscribedText(voiceNode: AccessibilityNodeInfo): String? {
        val newRoot = service.rootInActiveWindow ?: return null
        val transcribedText = findTranscribedTextBelowVoice(voiceNode, newRoot)
...
1. **从下面开始转写**，因为转写后语音消息下面会多出文字，会影响上面消息的位置
2. **每条语音之间等 1 秒**，避免操作太快
3. **如果连续失败 3 条**，就停止，记录日志，不要一直试
4. **转写完成后**，转出来的文字是灰色气泡，下面还有个"点击收起"按钮

---

**文档版本：** 开发指南 924 版 | 2026-09-24
**包含内容：** v2.0 + 托管流程 + QQ适配器 + 陌陌适配器 + Soul适配器 + 消息类型识别 + 语音图片处理 + QQ语音转文字 + Soul语音转文字 + 增强功能
